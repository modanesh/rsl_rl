...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu", urdf_manager=None):
        ...
        if urdf_manager is not None:
            ...
            # Ensure inject_urdf_to_encoder is set
            if 'inject_urdf_to_encoder' not in self.policy_cfg:
                self.policy_cfg['inject_urdf_to_encoder'] = 'concat'  # default
                print(f"[INFO - PPO] URDF mode not specified, defaulting to 'concat'")

            print(f"[INFO - PPO] Using URDF mode: {self.policy_cfg['inject_urdf_to_encoder']}")
        else:
            # Ensure inject_urdf_to_encoder is 'none' if no manager
            if 'inject_urdf_to_encoder' not in self.policy_cfg:
                self.policy_cfg['inject_urdf_to_encoder'] = 'none'
            if self.policy_cfg.get('inject_urdf_to_encoder', 'none') != 'none':
                print(f"[WARNING - PPO] inject_urdf_to_encoder is set to '{self.policy_cfg['inject_urdf_to_encoder']}' but no urdf_manager provided. Setting to 'none'.")
                self.policy_cfg['inject_urdf_to_encoder'] = 'none'

        # Extract type_to_name_map if using URDF features
        self.type_to_name_map = None
        if urdf_manager is not None and self.policy_cfg.get('inject_urdf_to_encoder', 'none') != 'none':
        ...
        use_embodiment_policy = (
                ...
                self.policy_cfg.get('inject_urdf_to_encoder', 'none') != 'none'
        )
        ...
        if isinstance(policy, ActorCriticEmbodiment):
            print(f"[INFO - PPO] Policy URDF mode: {policy.inject_urdf_to_encoder}")
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        for it in trange(start_iter, tot_iter):
            ...
            # Store actions for the entire batch to log distributions later
            batch_actions_list = []
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # We clone to ensure we keep the value as it was at this step
                    batch_actions_list.append(actions.clone())
                ...
                # --- MODIFICATION START: Compute Stats ---
                # Concatenate all steps: shape (num_steps * num_envs, num_actions)
                flat_actions = torch.cat(batch_actions_list, dim=0)

                # Calculate stats along batch dimension (dim=0)
                # These variables will be picked up by locals() in the log function
                action_means = flat_actions.mean(dim=0)
                action_stds = flat_actions.std(dim=0)
                action_mins = flat_actions.min(dim=0).values
                action_maxs = flat_actions.max(dim=0).values
                # --- MODIFICATION END ---
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        # # Check if the stats exist in locals
        # if "action_means" in locs:
        #     # Iterate over the action dimensions (should be 12)
        #     for dim in range(locs["action_means"].shape[0]):
        #         self.writer.add_scalar(f"ActionDist/Mean_dim_{dim}", locs["action_means"][dim].item(), locs["it"])
        #         self.writer.add_scalar(f"ActionDist/Std_dim_{dim}", locs["action_stds"][dim].item(), locs["it"])
        #         self.writer.add_scalar(f"ActionDist/Min_dim_{dim}", locs["action_mins"][dim].item(), locs["it"])
        #         self.writer.add_scalar(f"ActionDist/Max_dim_{dim}", locs["action_maxs"][dim].item(), locs["it"])
